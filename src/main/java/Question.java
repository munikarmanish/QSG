...
public class Question extends Timestamped {
    ...
    public static List<Question> limit(int start_index, int size) {
        try (Connection con = DB.sql2o.open()) {
            return con.createQuery("SELECT * FROM questions ORDER BY RAND() LIMIT :start, :size")
    ...
    public static List<Question> limitset(int dura, int diffi) {
        try (Connection con = DB.sql2o.open()) { 
            float f= (dura/4)*3;
            int number = Math.round(f);
            return con.createQuery("SELECT DISTINCT * FROM questions WHERE difficulty= :diffi ORDER BY RAND() LIMIT :number")
            .addParameter("number", number)
            .addParameter("diffi", diffi)
                .executeAndFetch(Question.class);
        }
    }
}
